...
<div class="container py-4">
    ...
    <div class="row g-4 mt-1">
        ...
        <div class="col-12"><div class="d-flex justify-content-between align-items-center mb-2"><strong>Perguntas</strong><button class="btn btn-sm btn-outline-primary" type="button" id="add-question">Adicionar pergunta</button></div><div id="question-list"><div class="border rounded p-3 mb-2 questionnaire-item"><label class="form-label">Texto da pergunta</label><input class="form-control mb-2" name="question_text[]" required><label class="form-label">Alternativas</label><input class="form-control" name="question_options[]" required placeholder="Sim, Não, Não sei"><div class="form-text">Separe as alternativas por vírgula. A pergunta será identificada como q1, q2 e assim por diante.</div></div></div></div>
        <div class="col-12"><div class="d-flex justify-content-between align-items-center mb-2"><strong>Regras de recomendação</strong><button class="btn btn-sm btn-outline-primary" type="button" id="add-rule">Adicionar regra</button></div><div id="rule-list"><div class="border rounded p-3 mb-2 rule-item"><div class="row g-2"><div class="col-6"><label class="form-label">Pergunta</label><input class="form-control" name="rule_question[]" required placeholder="q1"></div><div class="col-6"><label class="form-label">Resposta</label><input class="form-control" name="rule_answer[]" required placeholder="Sim"></div><div class="col-12"><label class="form-label">Produtos recomendados</label><input class="form-control" name="rule_products[]" required placeholder="Produto A, Produto B"><div class="form-text">Separe mais de um produto por vírgula.</div></div><div class="col-12"><label class="form-label">Por que recomendar?</label><textarea class="form-control" name="rule_reason[]" rows="2" required></textarea></div></div></div></div></div><div class="col-12"><button class="btn btn-primary">Salvar rascunho</button></div></form></div><div class="list-group list-group-flush"><?php foreach ($questionnaireVersions as $questionnaire): ?><div class="list-group-item d-flex justify-content-between gap-3"><div><strong><?= esc($questionnaire['title']) ?></strong><br><small><?= esc($questionnaire['campaign_name'] . ' · ' . $questionnaire['version']) ?></small></div><div class="text-end"><span class="badge text-bg-<?= $questionnaire['status'] === 'published' ? 'success' : 'secondary' ?>"><?= esc($questionnaire['status']) ?></span><?php if ($questionnaire['status'] === 'draft'): ?><form method="post" action="<?= site_url('admin/vendedor-eventual/catalogo/questionario/' . $questionnaire['id'] . '/publicar') ?>" class="mt-2"><?= csrf_field() ?><button class="btn btn-sm btn-success">Publicar</button></form><?php endif ?></div></div><?php endforeach ?></div></div></div>
    </div>
</div>
<script>
document.addEventListener('DOMContentLoaded', () => {
    const addItem = (listId, selector) => {
        const list = document.getElementById(listId);
        const item = list.querySelector(selector).cloneNode(true);
        item.querySelectorAll('input, textarea').forEach(field => field.value = '');
        const remove = document.createElement('button');
        remove.type = 'button'; remove.className = 'btn btn-sm btn-outline-danger mt-2'; remove.textContent = 'Remover';
        remove.addEventListener('click', () => item.remove()); item.appendChild(remove); list.appendChild(item);
    };
    document.getElementById('add-question')?.addEventListener('click', () => addItem('question-list', '.questionnaire-item'));
    document.getElementById('add-rule')?.addEventListener('click', () => addItem('rule-list', '.rule-item'));
});
</script>
<?= $this->endSection() ?>
