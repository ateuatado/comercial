# ============================================================
# Continuação v3 - Estabelecimentos, Sócios, Simples
# Usa arquivos .sql temporários para evitar problemas de parsing
# ============================================================
param(
...
function Run-SqlFile {
    ...
    $tmpFile = Join-Path $TempDir "_tmp_cmd.sql"
    Set-Content -Path $tmpFile -Value $Content -Encoding UTF8
    Run-Psql -Sql $tmpFile -File
}
...
function Log-Ingestao {
    ...
    Run-Psql "INSERT INTO receita.log_ingestao (tabela, registros_antes, registros_novos, registros_total, duracao_seg, status) VALUES ('$Tabela', $Antes, $Novos, $Total, $Duracao, '$Status');"
}

Write-Log "========== CONTINUACAO v3 =========="
...
# Limpar staging
Run-Psql "DROP TABLE IF EXISTS receita.stg_estabelecimentos;"
Run-Psql "DROP TABLE IF EXISTS receita.stg_socios;"
Run-Psql "DROP TABLE IF EXISTS receita.stg_simples;"
...
Write-Log "Estado atual: estab=$countEstab socios=$countSocios simples=$countSimples"
...
if ($countEstab -eq 0) {
    ...
    Write-Log "  Dropando indice GIN..."
    Run-Psql "DROP INDEX IF EXISTS receita.idx_estab_nome_fantasia;"

    Write-Log "  Criando staging..."
    Run-SqlFile "DROP TABLE IF EXISTS receita.stg_estabelecimentos;
CREATE TABLE receita.stg_estabelecimentos (
    ...
    Run-SqlFile "DELETE FROM receita.stg_estabelecimentos a USING receita.stg_estabelecimentos b WHERE a.ctid < b.ctid AND a.cnpj_basico = b.cnpj_basico AND a.cnpj_ordem = b.cnpj_ordem AND a.cnpj_dv = b.cnpj_dv;"

    Write-Log "  INSERT direto..."
    Run-SqlFile "INSERT INTO receita.estabelecimentos ($estabCols) SELECT $estabCols FROM receita.stg_estabelecimentos;"

    Run-Psql "DROP TABLE receita.stg_estabelecimentos;"

    Write-Log "  Recriando indice GIN..."
    ...
    $sw.Stop()
    $mins = [math]::Round($sw.Elapsed.TotalSeconds/60, 1)
    Log-Ingestao -Tabela "estabelecimentos" -Antes 0 -Novos $total -Total $total -Duracao $sw.Elapsed.TotalSeconds
    Write-Log "  ESTABELECIMENTOS: $total registros (${mins}min)"
} else {
    Write-Log "  ESTABELECIMENTOS: ja carregado ($countEstab). Pulando."
}
...
if ($countSocios -eq 0) {
    ...
    Write-Log "  Criando staging..."
    Run-SqlFile "DROP TABLE IF EXISTS receita.stg_socios;
CREATE TABLE receita.stg_socios (
    ...
    Write-Log "  Deduplicando staging..."
    Run-SqlFile "DELETE FROM receita.stg_socios a USING receita.stg_socios b WHERE a.ctid < b.ctid AND a.cnpj_basico = b.cnpj_basico AND a.cpf_cnpj_socio = b.cpf_cnpj_socio AND a.qualificacao_socio = b.qualificacao_socio;"

    Write-Log "  INSERT direto (sem indice GIN)..."
    Run-SqlFile "INSERT INTO receita.socios ($socioCols) SELECT $socioCols FROM receita.stg_socios;"

    Run-Psql "DROP TABLE receita.stg_socios;"

    Write-Log "  Recriando indice GIN nome_socio..."
    ...
    Write-Log "  SOCIOS: $total registros (${mins}min)"
} else {
    Write-Log "  SOCIOS: ja carregado ($countSocios). Pulando."
}
...
if ($countSimples -eq 0) {
    ...
    Write-Log "  Criando staging..."
    Run-SqlFile "DROP TABLE IF EXISTS receita.stg_simples;
CREATE TABLE receita.stg_simples (
    ...
    Write-Log "  Deduplicando staging..."
    Run-SqlFile "DELETE FROM receita.stg_simples a USING receita.stg_simples b WHERE a.ctid < b.ctid AND a.cnpj_basico = b.cnpj_basico;"

    Write-Log "  INSERT direto..."
    Run-SqlFile "INSERT INTO receita.simples ($simplesCols) SELECT $simplesCols FROM receita.stg_simples;"
    ...
    Write-Log "  SIMPLES: $total registros (${mins}min)"
} else {
    Write-Log "  SIMPLES: ja carregado ($countSimples). Pulando."
}

# ── ANALYZE + RESUMO ─────────────────────────────────────────
Write-Log "Atualizando estatisticas..."
...
$resumoSql = "SELECT 'empresas' AS tabela, count(*) AS registros FROM receita.empresas UNION ALL SELECT 'estabelecimentos', count(*) FROM receita.estabelecimentos UNION ALL SELECT 'socios', count(*) FROM receita.socios UNION ALL SELECT 'simples', count(*) FROM receita.simples UNION ALL SELECT 'cnaes', count(*) FROM receita.cnaes UNION ALL SELECT 'motivos', count(*) FROM receita.motivos UNION ALL SELECT 'municipios', count(*) FROM receita.municipios UNION ALL SELECT 'naturezas', count(*) FROM receita.naturezas UNION ALL SELECT 'paises', count(*) FROM receita.paises UNION ALL SELECT 'qualificacoes', count(*) FROM receita.qualificacoes ORDER BY tabela;"
Run-Psql $resumoSql

Write-Log "========== INGESTAO CONCLUIDA =========="
