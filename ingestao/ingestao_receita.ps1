...
# ============================================================
# Uso:   .\ingestao_receita.ps1
# Local: c:\xampp\htdocs\spiv\ingestao\
# Reqs:  PostgreSQL 18 (psql + pg_isready no PATH)
# ============================================================

param(
    [string]$ZipPath     = "c:\xampp\htdocs\spiv\adm\download.zip",
    [string]$TempDir     = "c:\xampp\htdocs\spiv\adm\temp_extract",
    [string]$SqlDir      = "c:\xampp\htdocs\spiv\sql",
...
function Run-Psql {
    ...
    if ($File) {
        & $PsqlExe -h $PgHost -p $PgPort -U $PgUser -d $PgDb -f $Sql -v ON_ERROR_STOP=1 2>&1 | ForEach-Object {
            if ($_ -is [System.Management.Automation.ErrorRecord]) {
                # Ignore PostgreSQL NOTICE/INFO messages
                $msg = $_.ToString()
                if ($msg -notmatch 'NOTA:|NOTICE:|INFO:') {
                    Write-Host "PSQL STDERR: $msg"
                }
            } else {
                Write-Host $_
            }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Erro psql (exit $LASTEXITCODE)"
    }
}

function Get-CsvFiles {
    param([string]$Pattern)
    return Get-ChildItem $TempDir -Filter $Pattern | Sort-Object Name
}
...
VALUES ('$Tabela', $Antes, $Novos, $Total, $Duracao, '$Status');
"@
    Run-Psql $sql
}
...
# ── ETAPA 1 e 2: Extrair ZIPs ───────────────────────────────

Write-Log "========== INÍCIO DA INGESTÃO =========="
Write-Log "ZIP: $ZipPath"

if ($SkipExtract) {
    Write-Log "Extração ignorada (-SkipExtract). Usando CSVs existentes em $TempDir"
} else {

if (-not (Test-Path $ZipPath)) {
    Write-Log "Arquivo $ZipPath não encontrado!" "ERROR"
    exit 1
}
...
Write-Log "Extraindo ZIP principal..."
$outerZip = [System.IO.Compression.ZipFile]::OpenRead($ZipPath)
foreach ($entry in $outerZip.Entries) {
    $dest = Join-Path $TempDir $entry.FullName
    if (Test-Path $dest) { Remove-Item $dest -Force }
    [System.IO.Compression.ZipFileExtensions]::ExtractToFile($entry, $dest, $true)
}
$outerZip.Dispose()
Write-Log "ZIP principal extraído."

# ── ETAPA 2: Extrair inner ZIPs ─────────────────────────────

Write-Log "Extraindo ZIPs internos..."
$innerZips = Get-ChildItem $TempDir -Filter "*.zip"
foreach ($iz in $innerZips) {
    Write-Log "  Extraindo $($iz.Name)..."
    try {
        $izObj = [System.IO.Compression.ZipFile]::OpenRead($iz.FullName)
        foreach ($e in $izObj.Entries) {
        ...
        Write-Log "  AVISO: Falha ao extrair $($iz.Name): $_" "WARN"
    }
}
Write-Log "ZIPs internos extraídos."

} # fim do if/else SkipExtract

# ── ETAPA 3: Criar schema (idempotente) ─────────────────────

Write-Log "Verificando/criando schema..."

# Habilitar extensão pg_trgm para índices GIN de busca textual
Run-Psql "CREATE EXTENSION IF NOT EXISTS pg_trgm;"

Run-Psql -Sql (Join-Path $SqlDir "01_criar_schema.sql") -File
Write-Log "Schema verificado/criado."

# ── ETAPA 4: Carregar tabelas de domínio ─────────────────────

Write-Log "=== Carregando tabelas de domínio ==="
...
foreach ($tabela in $dominioMap.Keys) {
    $pattern = $dominioMap[$tabela]
    $csvFile = Get-ChildItem $TempDir -Filter $pattern | Select-Object -First 1

    if (-not $csvFile) {
        Write-Log "  AVISO: Arquivo para $tabela não encontrado (padrão: $pattern)" "WARN"
        continue
    }

    $sw = [System.Diagnostics.Stopwatch]::StartNew()
    Write-Log "  Carregando $tabela de $($csvFile.Name)..."

    $antes = 0
    try { $antes = Get-TableCount $tabela } catch {}

    # TRUNCATE + COPY
    Run-Psql "TRUNCATE TABLE receita.$tabela;"

    Run-Copy -Table "receita.$tabela" -Columns "codigo, descricao" -FilePath $csvFile.FullName

    $total = Get-TableCount $tabela
    $sw.Stop()
    $novos = $total - $antes
    if ($novos -lt 0) { $novos = $total }

    Log-Ingestao -Tabela $tabela -Antes $antes -Novos $novos -Total $total -Duracao $sw.Elapsed.TotalSeconds
    Write-Log "  $tabela`: $total registros ($([math]::Round($sw.Elapsed.TotalSeconds, 1))s)"
}

# ── ETAPA 5: Carregar tabelas de dados (staging + upsert) ───

Write-Log "=== Carregando tabelas de dados ==="
...
function Load-Empresas {
    $sw = [System.Diagnostics.Stopwatch]::StartNew()
    $antes = 0
    try { $antes = Get-TableCount "empresas" } catch {}

    Write-Log "  Criando staging empresas..."
    Run-Psql @"
DROP TABLE IF EXISTS receita.stg_empresas;
...
"@
    ...
    $csvFiles = Get-ChildItem $TempDir -Filter "*EMPRECSV" | Sort-Object Name
    foreach ($f in $csvFiles) {
        Write-Log "  COPY $($f.Name)..."
        Run-Copy -Table "receita.stg_empresas" -Columns "cnpj_basico, razao_social, natureza_juridica, qualificacao_responsavel, capital_social, porte_empresa, ente_federativo" -FilePath $f.FullName
    }

    Write-Log "  Deduplicando staging empresas..."
    Run-Psql @"
DELETE FROM receita.stg_empresas a USING receita.stg_empresas b
WHERE a.ctid < b.ctid AND a.cnpj_basico = b.cnpj_basico;
"@

    $countTarget = 0
    try { $countTarget = Get-TableCount "empresas" } catch {}

    if ($countTarget -eq 0) {
        Write-Log "  INSERT direto empresas (tabela vazia)..."
        Run-Psql @"
INSERT INTO receita.empresas (cnpj_basico, razao_social, natureza_juridica, qualificacao_responsavel, capital_social, porte_empresa, ente_federativo)
SELECT cnpj_basico, razao_social, natureza_juridica, qualificacao_responsavel, capital_social, porte_empresa, ente_federativo
FROM receita.stg_empresas;
"@
    } else {
        Write-Log "  UPSERT empresas (incremental)..."
        Run-Psql @"
INSERT INTO receita.empresas (cnpj_basico, razao_social, natureza_juridica, qualificacao_responsavel, capital_social, porte_empresa, ente_federativo)
SELECT cnpj_basico, razao_social, natureza_juridica, qualificacao_responsavel, capital_social, porte_empresa, ente_federativo
FROM receita.stg_empresas
...
"@
    ...
    Run-Psql "DROP TABLE receita.stg_empresas;"

    $total = Get-TableCount "empresas"
    $sw.Stop()
    $novos = $total - $antes
    if ($novos -lt 0) { $novos = 0 }

    Log-Ingestao -Tabela "empresas" -Antes $antes -Novos $novos -Total $total -Duracao $sw.Elapsed.TotalSeconds
    Write-Log "  EMPRESAS: antes=$antes novos=$novos total=$total ($([math]::Round($sw.Elapsed.TotalSeconds, 1))s)"
}
...
SELECT DISTINCT ON (cnpj_basico, cnpj_ordem, cnpj_dv)
    ...
    ddd_1, telefone_1, ddd_2, telefone_2, ddd_fax, fax,
    email, situacao_especial, data_situacao_especial
FROM receita.stg_estabelecimentos
...
"@
    ...
    foreach ($f in $csvFiles) {
        Write-Log "  COPY $($f.Name)..."
        Run-Copy -Table "receita.stg_socios" -Columns "cnpj_basico, identificador_socio, nome_socio, cpf_cnpj_socio, qualificacao_socio, data_entrada_sociedade, pais, representante_legal, nome_representante, qualificacao_representante, faixa_etaria" -FilePath $f.FullName
    }

    Write-Log "  UPSERT socios..."
    $upsertSql = @"
...
SELECT DISTINCT ON (cnpj_basico, cpf_cnpj_socio, qualificacao_socio)
    cnpj_basico, identificador_socio, nome_socio, cpf_cnpj_socio,
    qualificacao_socio, data_entrada_sociedade, pais,
    representante_legal, nome_representante, qualificacao_representante, faixa_etaria
FROM receita.stg_socios
...
"@
    ...
    foreach ($f in $csvFiles) {
        Write-Log "  COPY $($f.Name)..."
        Run-Copy -Table "receita.stg_simples" -Columns "cnpj_basico, opcao_simples, data_opcao_simples, data_exclusao_simples, opcao_mei, data_opcao_mei, data_exclusao_mei" -FilePath $f.FullName
    }

    Write-Log "  UPSERT simples..."
    $upsertSql = @"
...
# Remover CSVs extraídos (manter ZIPs internos caso queira re-processar)
Get-ChildItem $TempDir -Exclude "*.zip" | Remove-Item -Force -ErrorAction SilentlyContinue
