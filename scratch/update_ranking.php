...
$newRankingApi = <<<'PHP'
    public function rankingApi()
    {
        ...
        // 2. Buscar estabelecimentos livres fora da carteira
        $rows = $db->query("
            SELECT
        ...
        // Carregar CNAEs em batch para os CNPJs retornados
        $cnpjs = array_column($rows, 'cnpj');
        $cnaesMap = $this->getBulkCnaesDetalhados($cnpjs);
        ...
        foreach ($rows as &$row) {
            ...
            if ($cnaePts == 0) $cnaePts = 15; // fallback base para empresa ativa
            $score   += $cnaePts;
            ...
            $row['cnaes_detalhados'] = $cnaesMap[$row['cnpj']] ?? [];
        }

        // Total de leads livres na base
        $total = (int) ($db->query("
            SELECT COUNT(*) AS c
            FROM receita.estabelecimentos e
            LEFT JOIN carteira_raw cr ON REGEXP_REPLACE(cr.cnpj, '[^0-9]', '', 'g') = (e.cnpj_basico || e.cnpj_ordem || e.cnpj_dv)
...
// Replace existing rankingApi method
$pattern = '/public function rankingApi\(\)\s*\{.*?\n    \}/s';
$content = preg_replace($pattern, trim($newRankingApi), $content);

file_put_contents($filePath, $content);
echo "Updated rankingApi successfully!\n";
